...
### Remote Services + Local App Containers
...
- local `ai-analyzer` container

use the production-style compose flow:

1. Copy `backend/.env.example` to `backend/.env`
2. Replace local DB/R2 placeholder values with your real remote values
3. Keep:
   - `POSTURE_ANALYZER_ENABLED=true`
   - `POSTURE_ANALYZER_URL=http://ai-analyzer:8000`
4. Run:
...
```bash
make deploy-remote-logs
make deploy-remote-down
make deploy-remote-build
```
...
and expects DB/R2/OpenAI to be remote.
