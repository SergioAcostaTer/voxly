...
public final class ResultMapper {
    ...
    public static <T> ResponseEntity<ApiResponse<T>> toAcceptedResponse(ResultT<T> result) {
        if (result.isSuccess()) {
            return ResponseEntity.status(HttpStatus.ACCEPTED).body(ApiResponse.ok(result.getValue()));
        }
        return toErrorResponse(result.getErrors());
    }
