...
public class SessionController {
    ...
    @PostMapping(value = "/{sessionId}/slides", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(summary = "Upload slide deck for a session")
    public ResponseEntity<ApiResponse<SessionResponse>> uploadSlides(
            @PathVariable UUID sessionId,
            @RequestParam("file") MultipartFile file
    ) {
        return ResultMapper.toResponse(sessionService.uploadSlides(sessionId, file));
    }
