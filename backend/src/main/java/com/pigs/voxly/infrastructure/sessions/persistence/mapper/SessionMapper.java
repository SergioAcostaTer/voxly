...
public class SessionMapper {
    ...
    public Session toDomain(SessionJpaEntity entity) {
        ...
        MediaFile slideFile = null;
        if (entity.getSlideStoragePath() != null) {
            slideFile = MediaFile.reconstitute(
                    entity.getSlideStoragePath(),
                    entity.getSlideOriginalFilename(),
                    entity.getSlideContentType(),
                    entity.getSlideSizeBytes() != null ? entity.getSlideSizeBytes() : 0,
                    null);
        }
    ...
    public SessionJpaEntity toEntity(Session session) {
        ...
        if (session.getSlideFile() != null) {
            entity.setSlideStoragePath(session.getSlideFile().getStoragePath());
            entity.setSlideOriginalFilename(session.getSlideFile().getOriginalFileName());
            entity.setSlideContentType(session.getSlideFile().getContentType());
            entity.setSlideSizeBytes(session.getSlideFile().getSizeBytes());
        }
