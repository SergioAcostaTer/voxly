...
public class EvaluationService {
    ...
    @Transactional
    public ResultT<EvaluationResponse> reanalyzePosture(UUID sessionId) {
        var userIdOpt = currentUserProvider.getUserId();
        if (userIdOpt.isEmpty()) {
            return ResultT.failure(SessionErrors.NOT_OWNER);
        }
        ...
        if (!sessionOpt.get().hasMedia()) {
            return ResultT.failure(EvaluationErrors.analysisFailed("Session has no media file"));
        }

        // Clear existing posture data so the frontend shows it as pending
        evaluation.setPostureResults(null, null, null, null, null, null, null);
        evaluationRepository.save(evaluation);
        sessionStatusStreamService.publishEvaluationUpdate(evaluation);

        evaluationProcessor.processPostureOnlyAsync(evaluation.getId().getValue());
