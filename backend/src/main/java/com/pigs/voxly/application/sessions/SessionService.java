...
public class SessionService {
    ...
    @Transactional
    public ResultT<SessionResponse> uploadSlides(UUID sessionId, MultipartFile file) {
        var userIdOpt = currentUserProvider.getUserId();
        if (userIdOpt.isEmpty()) {
            return ResultT.failure(SessionErrors.NOT_OWNER);
        }
        ...
        var validationResult = FileValidator.validateDocument(
        ...
        var session = sessionOpt.get();
        var uploadResult = storeAndAttachSlides(session, file);
        if (uploadResult.isFailure()) {
            return ResultT.failure(uploadResult.getError());
        }
    ...
    private Result storeAndAttachSlides(Session session, MultipartFile file) {
        try (var inputStream = file.getInputStream()) {
            var directory = "sessions/" + session.getId().getValue() + "/slides";
            ...
            var slideFile = MediaFile.create(
                    storeResult.getValue(),
                    normalizedFileName,
                    file.getContentType(),
                    file.getSize());

            var uploadResult = session.uploadSlides(slideFile);
            if (uploadResult.isFailure()) {
                storageService.delete(storeResult.getValue());
                return Result.failure(uploadResult.getError());
            }
            ...
            return Result.failure(com.pigs.voxly.sharedKernel.domain.results.Error.failure(
                    "Session.SlideUploadFailed", "Failed to upload slides: " + e.getMessage()));
        }
    }
