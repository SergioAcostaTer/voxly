...
public class EvaluationProcessor {
    ...
    public EvaluationProcessor(
            ...
            PostureAnalysisService postureAnalysisService,
    ...
    private void process(UUID evaluationId) {
        ...
        try {
            ...
            // Save speech analysis results immediately so the frontend shows them
            try {
                ...
                log.info("Speech analysis completed for: {}", evaluationId);
            } catch (JsonProcessingException e) {
                failEvaluation(evaluationId, "Failed to serialize analysis results");
                return;
            }

            // Step 3: Posture analysis (truly non-blocking — speech results already saved)
            log.info("Starting posture analysis for evaluation: {}", evaluationId);
            var postureResult = postureAnalysisService.analyze(mediaPath, progress -> {
                sessionStatusStreamService.publishPostureProgress(
                        evaluation.getUserId().getValue(),
                        evaluation.getSessionId().getValue(),
                        evaluationId,
                        progress);
            });
            if (postureResult.isSuccess()) {
                var posture = postureResult.getValue();
                try {
                    String recommendationsJson = objectMapper.writeValueAsString(posture.recommendations());
                    evaluation.setPostureResults(
                            posture.finalScore(),
                            posture.grade(),
                            posture.gestureSummariesJson(),
                            posture.timelineJson(),
                            posture.penaltyBreakdownJson(),
                            recommendationsJson,
                            posture.renderedVideoUrl());
                    evaluationRepository.save(evaluation);
                    sessionStatusStreamService.publishEvaluationUpdate(evaluation);
                    log.info("Posture analysis completed: score={} grade={}", posture.finalScore(), posture.grade());
                } catch (JsonProcessingException e) {
                    log.warn("Failed to serialize posture recommendations, skipping posture data");
                }
            } else {
                log.warn("Posture analysis failed (non-fatal): {}", postureResult.getError().getMessage());
            }

            sendCompletionNotification(evaluation.getUserId().getValue(), session.getTitle().getValue(), session.getId().getValue());
    ...
    @Async("analysisExecutor")
    @Transactional
    public void processPostureOnlyAsync(UUID evaluationId) {
        if (activeEvaluations.putIfAbsent(evaluationId, Boolean.TRUE) != null) {
            log.info("Skipping posture-only processing for {} because it is already active", evaluationId);
            return;
        }

        log.info("Starting posture-only analysis for: {}", evaluationId);

        try {
            processPostureOnly(evaluationId);
        } catch (Exception e) {
            log.error("Failed to process posture for evaluation: {}", evaluationId, e);
        } finally {
            activeEvaluations.remove(evaluationId);
        }
    }

    private void processPostureOnly(UUID evaluationId) {
        var evaluationOpt = evaluationRepository.findById(EvaluationId.from(evaluationId));
        if (evaluationOpt.isEmpty()) {
            log.error("Evaluation not found for posture re-analysis: {}", evaluationId);
            return;
        }

        var evaluation = evaluationOpt.get();
        var sessionOpt = sessionRepository.findById(evaluation.getSessionId());
        if (sessionOpt.isEmpty() || !sessionOpt.get().hasMedia()) {
            log.warn("Session has no media for posture re-analysis: {}", evaluationId);
            return;
        }
        ...
        try {
            ...
            log.info("Posture re-analysis via HTTP for: {}", mediaPath.getFileName());
            var postureResult = postureAnalysisService.analyze(mediaPath, progress -> {
                sessionStatusStreamService.publishPostureProgress(
                        evaluation.getUserId().getValue(),
                        evaluation.getSessionId().getValue(),
            ...
            if (postureResult.isSuccess()) {
                ...
                evaluationRepository.save(evaluation);
                sessionStatusStreamService.publishEvaluationUpdate(evaluation);
                log.info("Posture re-analysis completed: score={} grade={}", posture.finalScore(), posture.grade());
            } else {
                log.warn("Posture re-analysis failed: {}", postureResult.getError().getMessage());
            }
        } catch (Exception e) {
            log.error("Posture re-analysis error: {}", evaluationId, e);
        } finally {
            storageService.cleanupTemporaryFile(mediaPath);
        }
    }
    ...
    private void sendCompletionNotification(UUID userId, String sessionTitle, UUID sessionId) {
        try {
            var userOpt = userRepository.findById(com.pigs.voxly.domain.identity.UserId.from(userId));
            if (userOpt.isEmpty()) {
                return;
            }

            var user = userOpt.get();
            emailService.sendAnalysisCompleted(
            ...
            log.warn("Failed to send analysis completion notification for session {}", sessionId, e);
        }
    }
}
