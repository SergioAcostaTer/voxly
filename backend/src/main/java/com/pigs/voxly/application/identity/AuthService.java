...
public class AuthService {
    ...
    public ResultT<UserResponse> register(RegisterRequest request) {
        ...
        user.updateProfessionalProfile(request.professionalRole(), request.coachingFocus());
    ...
    public Result changePassword(UUID userId, String currentPassword, String newPassword) {
        ...
        if (!passwordHasher.verify(currentPassword, user.getPasswordHash().getValue())) {
        ...
        user.changePassword(passwordHashResult.getValue());
        user.revokeAllRefreshTokens();
        userRepository.save(user);
        publishAndClear(user);
    ...
    @Transactional
    public ResultT<LoginResponse> loginWithExternalProvider(String emailValue, String displayName) {
        var emailResult = Email.create(emailValue);
        if (emailResult.isFailure()) {
            return ResultT.failure(emailResult.getErrors());
        }
        ...
        if (userOpt.isPresent()) {
            ...
            var usernameResult = createUniqueExternalUsername(emailValue, displayName);
            if (usernameResult.isFailure()) {
                return ResultT.failure(usernameResult.getErrors());
            }

            String generatedPassword = UUID.randomUUID().toString() + UUID.randomUUID();
            var passwordHashResult = PasswordHash.create(passwordHasher.hash(generatedPassword));
            if (passwordHashResult.isFailure()) {
                return ResultT.failure(passwordHashResult.getErrors());
            }

            var userResult = User.create(email, usernameResult.getValue(), passwordHashResult.getValue());
            if (userResult.isFailure()) {
                return ResultT.failure(userResult.getErrors());
            }
    ...
    private ResultT<Username> createUniqueExternalUsername(String emailValue, String displayName) {
        String seed = displayName != null && !displayName.isBlank()
                ? displayName
                : emailValue.substring(0, emailValue.indexOf('@'));
        String base = seed.toLowerCase().replaceAll("[^a-z0-9]", "");
        if (base.length() < Username.MIN_LENGTH) {
            base = "user" + base;
        }
        if (base.length() > 42) {
        ...
        for (int attempt = 0; attempt < 50; attempt++) {
            ...
            var usernameResult = Username.create(candidate);
            if (usernameResult.isFailure()) {
                return usernameResult;
            }
            if (!userRepository.existsByUsername(usernameResult.getValue())) {
                return usernameResult;
            }
        }

        return Username.create("user" + UUID.randomUUID().toString().replace("-", "").substring(0, 12));
    }
