...
public class SessionStatusStreamService {
    ...
    public SseEmitter subscribe(UUID userId, UUID sessionId) {
        ...
        send(emitter, "connected", new SessionStatusEvent(sessionId, null, null, null, null, Instant.now(), "connected"));
        return emitter;
    }
    ...
    public void publishPostureProgress(UUID userId, UUID sessionId, UUID evaluationId, double progress) {
        emitByKey(userId, sessionId, "posture_progress", new SessionStatusEvent(
                sessionId,
                null,
                evaluationId,
                null,
    ...
    private void emitByKey(UUID userId, UUID sessionId, String eventName, SessionStatusEvent event) {
        SseEmitter emitter = emitters.get(key(userId, sessionId));
        if (emitter == null) {
            return;
        }
